...
public final class DownloadProcessController: @unchecked Sendable {
    ...
    public static func terminateProcessTree(_ proc: Process?, pid: pid_t? = nil) {
        ...
        // Gather descendants before terminating the parent so already-reparented
        // children cannot escape the shutdown sweep.
        ...
        if resolvedPID > 0 {
            ...
            // siphon-pgrp calls setpgid(0, 0) before exec(), so the launched
            // executable remains the process-group leader and pgid == PID.
            // Only group-kill when that invariant is proven. Otherwise a PID
            // sweep is safer than broadcasting into an unrelated process group.
        ...
        if resolvedPID > 0 {
            // Multi-pass sweep catches children spawned during shutdown without
            // relying on process-group membership when it has not been proven safe.
            for _ in 0..<2 {
                usleep(25_000)
                for child in getDescendantPIDs(for: resolvedPID) {
            ...
            try await withCheckedThrowingContinuation { continuation in
                ...
                process.terminationHandler = { proc in
                    ...
                    if verifiedFinalPaths.isEmpty {
                        for candidate in outputState.getCandidatePaths().reversed() {
                    ...
                    if proc.terminationStatus == 0 {
                        if let primary = verifiedFinalPaths.first {
                            ...
                            safeContinuation.resume(throwing: YtdlpError.downloadFailed("Download process completed, but no valid media file was verified in the target destination."))
                              ...
                              lower.contains("sign in to confirm you’re not a bot") ||
                              (lower.contains("403") && (lower.contains("anti-bot") || lower.contains("bot detection") || lower.contains("captcha") || lower.contains("challenge"))) {
                    ...
                    } else if lower.contains("subtitle") || lower.contains("caption") {

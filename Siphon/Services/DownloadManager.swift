...
class DownloadManager: ObservableObject {
    ...
    private func executeDownload(_ download: Download) async {
        ...
        do {
            guard try await fetchDownloadInfo(for: download) else { return }
            ...
            let outputPath = try await performDownload(for: download)
            finalizeCompletedDownload(download, outputPath: outputPath)

        } catch let error as YtdlpError {
            handleDownloadYtdlpError(error, for: download)
        } catch {
            handleDownloadGenericError(error, for: download)
        }
    }

    private func fetchDownloadInfo(for download: Download) async throws -> Bool {
        let info = try await ytdlpService.fetchInfo(url: download.url, rawCookies: download.options.rawCookies)

        guard !Task.isCancelled else { return false }
        guard download.status == .fetching else { return false }
    ...
    private func handleDownloadYtdlpError(_ error: YtdlpError, for download: Download) {
    ...
    private func handleDownloadGenericError(_ error: Error, for download: Download) {
        if download.status == .stopped || download.status == .paused || Task.isCancelled {
            LoggerService.shared.log("Download stopped or paused by user (\(LoggerService.sanitizeURLForLog(download.url)))", level: .info)
            if download.status == .stopped {
                addToHistory(download)

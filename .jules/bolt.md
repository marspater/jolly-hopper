...
**Learning:** Using `errorOutput.split(whereSeparator: \.isNewline).reversed()` allocates an intermediate `Array<Substring>` containing all lines before reversing. Prepending `.lazy` (`errorOutput.lazy.split(whereSeparator: \.isNewline).reversed()`) does not avoid this allocation: `.reversed()` requires a `BidirectionalCollection`, so the lazy split sequence still has to be buffered into an array-backed collection before it can be traversed in reverse. Manually scanning backward with `String.Index` (finding the last newline before the current search position) avoids materializing any line array and stops as soon as a matching line is found.
**Action:** When searching backwards for specific log or error lines in multiline string outputs, scan manually with `String.Index`/`lastIndex(where:)` instead of relying on `.lazy.split(...).reversed()`, which still allocates a buffer.

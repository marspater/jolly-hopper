...
## $(date +%Y-%m-%d) - Pre-compile regexes in loops
**Learning:** Initializing `NSRegularExpression` with patterns inside a high-frequency loop is extremely costly in Swift, causing performance degradation due to redundant compilation and allocations.
**Action:** Extract repetitive regex pattern initializations out of the loop and define them as `nonisolated private static let` properties in the enclosing class or struct. This ensures they are pre-compiled once and reused across all function invocations, yielding a ~2x performance improvement in benchmarked parsing loops.

...
## 2026-09-15 - Optimize array allocations when processing string lines
**Learning:** `components(separatedBy: .newlines)` eagerly allocates a potentially large intermediate array of `String` instances on the heap. Using `split(whereSeparator: \.isNewline)` is far more efficient as it operates lazily and generates `Substring` instances mapped to the original string's buffer.
**Action:** Always prefer `split(whereSeparator: \.isNewline)` over `components(separatedBy: .newlines)` when iterating through string lines or segments.

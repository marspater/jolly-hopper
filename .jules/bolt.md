...
## 2026-08-15 - Lazy Mapping Before Set Initialization
**Learning:** Initializing a `Set` from `array.map { ... }` creates an intermediate eager array allocation before constructing the hash set. Using `array.lazy.map { ... }` streams elements directly into `Set.init(_:)`, avoiding intermediate array allocations and reducing peak memory usage.
**Action:** Prefer `Set(collection.lazy.map { ... })` over `Set(collection.map { ... })` when extracting identifiers or property sets from arrays.

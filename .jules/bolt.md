...
## 2026-08-19 - Pre-compiling NSRegularExpression Patterns
**Learning:** Constructing `NSRegularExpression(pattern:options:)` inside loop iterations or frequently called methods causes heavy regex pattern parsing and object allocation overhead. Moving static patterns to `nonisolated private static let` properties eliminates repeated compilation during execution.
**Action:** Always pre-compile static regex patterns into static properties or arrays rather than calling `NSRegularExpression` inside loops or hot paths.

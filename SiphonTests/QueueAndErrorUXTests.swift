...
final class QueueAndErrorUXTests: XCTestCase {
    ...
    // MARK: - Dedicated Download Model Unit Tests

    func testDownloadInitializationAndHTMLEntityDecoding() {
        let options = DownloadOptions.default
        let rawTitle = "Tom &amp; Jerry &#39;The Movie&#39; &quot;Special&quot;"
        let download = Download(url: "https://example.com/video", options: options, title: rawTitle)

        XCTAssertEqual(download.title, "Tom & Jerry 'The Movie' "Special"", "Download init must decode HTML entities in title")
        XCTAssertEqual(download.url, "https://example.com/video")
        XCTAssertEqual(download.status, .queued)
    ...
    func testDownloadDisplayProgressFormatting() {
        let download = Download(url: "https://example.com/video", options: .default)

        // Basic progress percentage without speed or ETA
        download.progress = 0.456
        XCTAssertEqual(download.displayProgress, "45%")

        // Progress percentage with speed and ETA
    ...
    func testHDRActionTitlesAndIdentifiers() {
        let lang = LanguageService()
